...
import { Box, Button, TextField, Typography, Link, Snackbar, Alert, CircularProgress } from "@mui/material";
...
function Login({ setIsAuthenticated }) {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const response = await axios.post("http://13.202.251.211/api/login", formData);
  ...
  return (
    <Box display="flex" justifyContent="center" alignItems="center" height="100vh" sx={{ background: "linear-gradient(135deg, #1e3c72, #2a5298)", color: "white", padding: 2 }}>
      <Box sx={{ width: { xs: "85%", sm: "350px" }, backgroundColor: "rgba(255, 255, 255, 0.1)", padding: 3, borderRadius: 3, boxShadow: "0 8px 16px rgba(0, 0, 0, 0.2)" }}>
        <Typography variant="h5" textAlign="center" marginBottom={2} color="white" fontWeight="bold">
          Login
        </Typography>
        <form onSubmit={handleLogin}>
          ...
          <Box display="flex" justifyContent="space-between" marginTop={2}>
            <Typography variant="body2" component="span" sx={{ cursor: "pointer", color: "#a0c4ff" }}>
              Forgot password?
            </Typography>
            <Link href="/register" underline="none" sx={{ color: "#a0c4ff", fontWeight: "bold" }}>
              Register
            </Link>
          </Box>
          <Button fullWidth variant="contained" sx={{ marginTop: 3, backgroundColor: "black", "&:hover": { backgroundColor: "#2a5298" } }} type="submit" disabled={loading} startIcon={loading && <CircularProgress size={20} color="inherit" />}>
